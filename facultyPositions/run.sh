...
export HARVEST_NAME=dat-faculty-positions
...
harvester-sparqltranslator -X facultyPositions.config.xml

Type the Faculty
harvester-sparqltranslator -X facultyType.config.xml
...
REGIONS=`cat data/constructed-data | grep 'geographicFocus rdf:resource="http://aims.fao.org/aos/geopolitical.owl#' | wc -l`


echo "Imported $REGIONS Regions for $FAC Faculty Members"
